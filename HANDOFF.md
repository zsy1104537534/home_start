...
* **Riot damage moved out of the `PP_GENERATE_RIOT_DAMAGE` flag in current master.** The apartment terrains there get riot damage from a `post_process_generators: [ "riot_damage" ]` entry (`data/json/post_process_generators.json`, `data/json/overmap/overmap_terrain/overmap_terrain_residential.json`), while the flag itself still exists in the engine (`src/omdata.h`, `src/overmap_terrain.cpp`). That is why this mod's `delete` of the flag is a **silent no-op** on experimental builds rather than an error. On 0.I the flag is still the mechanism, and 0.I rejects the new field — so supporting both would take two files.
* **The companion patch mod covers experimental builds.** `home_start_riot_patch/overmap_terrain.json` repeats the twelve interior ids with `copy-from: apartments_tower_any` and `delete: { "post_process_generators": [ "riot_damage" ] }` — the same pattern upstream uses itself (`generic_city_house_roof` in the same data file deletes the inherited generator). Two things were verified on `0.I-1`: the sibling mod's name and description render in Chinese (every `*.mo` under the user mod directory is loaded into one catalogue), and **enabling it on 0.I causes no error** — `--check-mods home_start_riot_patch` exits 0 on the second pass, i.e. deleting a member those versions do not have is silently ignored. Whether the delete really removes riot damage on an experimental build is **not yet verified** (no experimental install here); the README says so.`n* **How mods are found and installed** (`src/mod_manager.cpp`): `mods/` is created automatically if missing (`assure_dir_exist( PATH_INFO::user_moddir() )` at startup), so a freshly unpacked game needs no manual setup; `modinfo.json` is searched **recursively** (`get_files_from_path( MOD_SEARCH_FILE, path, true )` in `load_mods_from`), which is why a nested "Download ZIP" folder such as `home_start-main/home_start/` still loads; and a **second** folder carrying the same `id` triggers `debugmsg( "there is already a mod with ident %s" )` — the old folder must be deleted when updating.
...
The shipped `.mo` is reproducible: compiling `zh_CN.po` with the gettext compiler that ships with Python (`Tools/i18n/msgfmt.py`) produces a **byte-identical** file — sha256 `0695a04dc64a725e2c5650cf63870024a7f28c4d3adabb79777ddddff51f55de`, 5464 bytes, 15 strings (9 of them contextual, the other two being the companion patch mod's name and description). No GNU `msgfmt` was available on the development machine, so that compiler check was run with the Python tool; note that GNU `msgfmt` writes a hash table by default, so its output can differ byte-wise while staying functionally identical.
